...
print(X)
y = dataset['L_timeslot 9-11']


train_data = pd.read_csv('../../Data/westminster.csv')
test_data = pd.read_csv('../../Data/westminster3.csv')

X_train = train_data.drop('L_timeslot 9-11', axis=1)
y_train = train_data['L_timeslot 9-11']

X_test = test_data.drop('L_timeslot 9-11', axis=1)
y_test = test_data['L_timeslot 9-11']
...
plt.title('Time VS (predicting) Motion factor', )
plt.xlabel('Time', )
plt.ylabel('Motion', )
plt.title('Multiple Linear MLR: Time vs Motion')
plt.grid(True)
plt.legend()
plt.show()
...
plt.plot(dataset['Time'], train_data['sensor_mo.mean'], color='green',  label='Motion')
