...
dataset_train = pd.read_csv('../../Data/UoW_Expediture_Sample.csv', header=0, index_col=0)
training_set = dataset_train.iloc[6:, 1:2].values # takes 1/3 of attendance from data
print(training_set)
...
dataset_total = pd.concat((dataset_train['Total_expenditure'], dataset_test['Total_expenditure']), axis=0)
...
result = adfuller(dataset_train['Total_expenditure'])
...
ax1.plot(dataset_train['Total_expenditure'])
ax1.set_title('Original Series')
ax1.axes.xaxis.set_visible(False)

# 1st Differencing
ax2.plot(dataset_train['Total_expenditure'].diff())
ax2.set_title('1st Order Differencing')
ax2.axes.xaxis.set_visible(False)

# 2nd Differencing
ax3.plot(dataset_train['Total_expenditure'].diff().diff())
...
fig, (ax1, ax2, ax3) = plt.subplots(3)
plot_acf(dataset_train['Total_expenditure'], ax=ax1)
plot_acf(dataset_train['Total_expenditure'].diff().dropna(), ax=ax2)
plot_acf(dataset_train['Total_expendituree'].diff().diff().dropna(), ax=ax3)


fig, (ax1, ax2, ax3) = plt.subplots(3)
plot_pacf(dataset_train['Total_expenditure'], ax=ax1)
plot_pacf(dataset_train['Total_expenditure'].diff().dropna(), ax=ax2)
plot_pacf(dataset_train['Total_expenditure'].diff().diff().dropna(), ax=ax3)
