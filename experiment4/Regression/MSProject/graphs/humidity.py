...
try:
    ...
    plt.plot(filtered_data['time'], filtered_data['humidity'], linestyle='--', color='blue')
    ...
    min_temp = filtered_data['humidity'].min()
    max_temp = filtered_data['humidity'].max()
    y_ticks = [i for i in range(int(min_temp), int(max_temp) + 1)]
    y_ticks_with_half = [tick + 0.5 for tick in y_ticks]
    plt.yticks(y_ticks + y_ticks_with_half)  # Combine integer and half ticks
